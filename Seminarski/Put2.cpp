...
void APut2::OnConstruction(const FTransform& Transforom)
{
	...
	{
	/*Possibly constructor can get wiled*/
	...
	FVector vvv(FMath::RandRange(-ProstorZaKrofne->GetUnscaledBoxExtent().X, ProstorZaKrofne->GetUnscaledBoxExtent().X), FMath::RandRange(-ProstorZaKrofne->GetUnscaledBoxExtent().Y, ProstorZaKrofne->GetUnscaledBoxExtent().Y)
		, FMath::RandRange(-ProstorZaKrofne->GetUnscaledBoxExtent().Z, ProstorZaKrofne->GetUnscaledBoxExtent().Z));

	vvv = vvv + ProstorZaKrofne->GetComponentLocation();
	//AActorBaseClass* NewActor = UFunctionLibrary::SpawnBP<AActorBaseClass>(GetWorld(), TheActorBluePrint, SpawnLoc, SpawnRot);
	//UClass *ada = kojikurac.Class.operator UClass *();
	FRotator RotacijaProstoraZaKrofne = ProstorZaKrofne->GetComponentRotation();
	//FVector vada(3, 3, 3);
	Krofna= Cast<AKrofna_CPP, AActor>(GetWorld()->SpawnActor(AKrofna_CPP::StaticClass(), &vvv, &RotacijaProstoraZaKrofne));
	//tmp->AttachRootComponentToActor(this);
	//tmp->AttachRootComponentToActor(this, NAME_None, EAttachLocation::KeepWorldPosition, false);

	//GetWorld()->SpawnActor(APut2::GetClass(), &LokacijaNadovezivanje, &RotacijaNadovezivanje);
